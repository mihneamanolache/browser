...
// A real 1x1 PNG generated by the canvas encoder. Image resource tests must
// exercise decoding, not just an HTTP 200 with an image/png header.
const image_test_png = blk: {
    const base64 = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4AWJiYGBgAAAAAP//XRcpzQAAAAZJREFUAwAADwADJDd96QAAAABJRU5ErkJggg==";
    const decoder = std.base64.standard.Decoder;
    var bytes: [decoder.calcSizeForSlice(base64) catch unreachable]u8 = undefined;
    decoder.decode(&bytes, base64) catch unreachable;
    break :blk bytes;
};
...
fn testHTTPHandler(req: *std.http.Server.Request) !void {
    ...
    if (std.mem.eql(u8, path, "/images/ok.png")) {
        return req.respond(&image_test_png, .{
            .extra_headers = &.{
    ...
    if (std.mem.eql(u8, path, "/images/bad.png")) {
        return req.respond("not a PNG", .{
            .extra_headers = &.{
                .{ .name = "Content-Type", .value = "image/png" },
            },
        });
    ...
    if (std.mem.eql(u8, path, "/beacon")) {
        ...
        // replies 204 and that is what the client expects to discard. Close
        // this test connection rather than attempting std.http.Server's
        // keep-alive body discard while a beacon is still being uploaded.
        return req.respond("", .{ .status = .no_content, .keep_alive = false });
    }
    ...
    if (std.mem.eql(u8, path, "/font_valid.ttf")) {
        return TestHTTPServer.sendFile(req, "src/rust/render/fonts/DejaVuSans.ttf");
    }
